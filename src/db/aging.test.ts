...
describe('getAgingReport', () => {
  it('excludes retailers with no outstanding balance', async () => {
    ...
    await createSale({ saleType: 'RETAIL', retailerId: retailer.id, items: [{ inventoryId: item.id, unit: 'kg', quantity: 5, salePrice: 60 }] });
  ...
  it('buckets a sale correctly based on days outstanding', async () => {
    ...
    const item = await setupProduct();
    const sale = await createSale({ saleType: 'RETAIL', retailerId: retailer.id, items: [{ inventoryId: item.id, unit: 'kg', quantity: 5, salePrice: 60 }] });
  ...
  it('computes totalOutstanding as the sum of unpaid amounts, not full sale totals', async () => {
    ...
    const item = await setupProduct();
    await createSale({ saleType: 'RETAIL', retailerId: retailer.id, items: [{ inventoryId: item.id, unit: 'kg', quantity: 10, salePrice: 60 }] }); // 600
  ...
  it('sorts retailers with the most overdue bucket first', async () => {
    ...
    const mildSale = await createSale({ saleType: 'RETAIL', retailerId: mild.id, items: [{ inventoryId: item.id, unit: 'kg', quantity: 5, salePrice: 60 }] });
    const severeSale = await createSale({ saleType: 'RETAIL', retailerId: severe.id, items: [{ inventoryId: item.id, unit: 'kg', quantity: 5, salePrice: 60 }] });
  ...
  it('multiple unpaid sales for the same retailer are sorted oldest-first within entries', async () => {
    ...
    const item = await setupProduct();
    const saleA = await createSale({ saleType: 'RETAIL', retailerId: retailer.id, items: [{ inventoryId: item.id, unit: 'kg', quantity: 5, salePrice: 60 }] });
    const saleB = await createSale({ saleType: 'RETAIL', retailerId: retailer.id, items: [{ inventoryId: item.id, unit: 'kg', quantity: 5, salePrice: 60 }] });
