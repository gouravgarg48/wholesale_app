...
describe('createReturn', () => {
  it('restores inventory by the returned amount', async () => {
    ...
    await createReturn({ saleId: sale.id, items: [{ inventoryId: item.id, unit: 'kg', quantity: 4 }], reason: 'Damaged bag' });
  ...
  it('computes refund using the sale-time price, not current market price', async () => {
    const item = await setupProduct();
    const sale = await createSale({ saleType: 'RETAIL', retailerId: 'r1', items: [{ inventoryId: item.id, unit: 'kg', quantity: 10, salePrice: 55 }] }); // sold below market price

    const ret = await createReturn({ saleId: sale.id, items: [{ inventoryId: item.id, unit: 'kg', quantity: 4 }], reason: 'Wrong item' });
  ...
  it('reduces a RETAIL sale\'s outstanding balance by the refund amount', async () => {
    const item = await setupProduct();
    const sale = await createSale({ saleType: 'RETAIL', retailerId: 'r1', items: [{ inventoryId: item.id, unit: 'kg', quantity: 10, salePrice: 60 }] }); // 600 owed

    await createReturn({ saleId: sale.id, items: [{ inventoryId: item.id, unit: 'kg', quantity: 5 }], reason: 'Excess order' }); // 300 credited
  ...
  it('does not touch amountPaid/paymentStatus for a CASH sale', async () => {
    const item = await setupProduct();
    const sale = await createSale({ saleType: 'CASH', buyerName: 'Walk-in', items: [{ inventoryId: item.id, unit: 'kg', quantity: 10, salePrice: 60 }] });

    await createReturn({ saleId: sale.id, items: [{ inventoryId: item.id, unit: 'kg', quantity: 3 }], reason: 'Customer changed mind' });
  ...
  it('rejects returning more than was sold', async () => {
    ...
    await expect(
      createReturn({ saleId: sale.id, items: [{ inventoryId: item.id, unit: 'kg', quantity: 15 }], reason: 'Too many' })
  ...
  it('rejects a second return that would exceed what remains after a prior return', async () => {
    ...
    await createReturn({ saleId: sale.id, items: [{ inventoryId: item.id, unit: 'kg', quantity: 6 }], reason: 'First return' });

    // only 4 remain returnable; asking for 5 should fail
    await expect(
      createReturn({ saleId: sale.id, items: [{ inventoryId: item.id, unit: 'kg', quantity: 5 }], reason: 'Second return' })
    ).rejects.toThrow(/remaining returnable/);
  });
  ...
  it('rejects a return against a cancelled sale', async () => {
    ...
    await expect(
      createReturn({ saleId: sale.id, items: [{ inventoryId: item.id, unit: 'kg', quantity: 1 }], reason: 'Too late' })
  ...
  it('rolls back inventory changes if one item in a multi-item return is invalid', async () => {
    ...
    await createRestock({ items: [{ inventoryId: itemB.id, unit: 'kg', quantity: 100, costPricePerUnit: 30 }] });
  ...
  it('rejects a return with an empty reason', async () => {
    const item = await setupProduct();
    const sale = await createSale({ saleType: 'RETAIL', retailerId: 'r1', items: [{ inventoryId: item.id, unit: 'kg', quantity: 10, salePrice: 60 }] });
    await expect(createReturn({ saleId: sale.id, items: [{ inventoryId: item.id, unit: 'kg', quantity: 1 }], reason: '  ' })).rejects.toThrow();
  });
