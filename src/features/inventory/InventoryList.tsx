            ...
            return (
              <tr key={item.id} className={item.quantity === 0 ? 'text-[#b54b3a]' : undefined}>
                ...
                <td className="px-3 py-2.5 border-b border-[#e4dccb] text-right whitespace-nowrap">
                  {isEditing ? (
                    <>
                      <button onClick={() => saveEdit(item.id)} disabled={saving} className="text-sm underline mr-2 disabled:opacity-50">
                        {saving ? 'Saving…' : 'Save'}
                      </button>
                      <button onClick={cancelEdit} disabled={saving} className="text-sm text-[#6b6555] underline">
                        Cancel
                      </button>
                    </>
                  ) : (
                    <button onClick={() => startEdit(item)} className="text-sm text-[#6b6555] underline">
                      Edit price
                    </button>
                  )}
                </td>
