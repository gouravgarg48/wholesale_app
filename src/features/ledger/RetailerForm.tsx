  ...
  return (
    <form className="max-w-lg mx-auto mb-8 p-6 bg-[#faf7f2] border border-[#e4dccb] font-sans text-[#2b2620]" onSubmit={handleSubmit}>
      ...
      <label className={fieldLabelClass}>
        Name
        <input className={inputClass} value={name} onChange={(e) => setName(e.target.value)} placeholder="Sharma Traders" />
      </label>

      <label className={fieldLabelClass}>
        Phone (optional)
        <input className={inputClass} value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="9999999999" />
      </label>

      <label className={fieldLabelClass}>
        Address (optional)
        <input className={inputClass} value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Main Market, ..." />
      </label>
